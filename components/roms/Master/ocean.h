...
!
#ifdef HAVE_MOAB
      USE moab_coupler_mod, ONLY: MOAB_initialize, MOAB_finalize, &
                                  MOAB_RegisterROMSApplications, &
                                  MOAB_DeregisterROMSApplications, &
                                  MOAB_AllocateGridData, &
                                  MOAB_DeallocateGridData, &
                                  MOAB_SetGrid, MOAB_SetGrid3D
#endif
!
      implicit none
!
...
#ifdef HAVE_MOAB
!     Initialize iMOAB
      call MOAB_initialize()

!     Let us now register the ROMS application
      call MOAB_RegisterROMSApplications(Ngrids)

      DO ng=1,Ngrids
!       Allocate all necessary data needed
        call MOAB_AllocateGridData(ng, first_tile(ng))

        DO tile=first_tile(ng),last_tile(ng),+1
          ! set the 2d grid data
          call MOAB_SetGrid(ng, tile)
#ifdef SOLVE3D
          ! set the 3d grid data
          call MOAB_SetGrid3D(ng, tile)
#endif
        END DO
      END DO
#endif
...
!     Deallocate all necessary data needed
      DO ng=1,Ngrids
        call MOAB_DeallocateGridData(ng)
      END DO

!     All done. de-register application
      call MOAB_DeregisterROMSApplications()
